...
static int ftdi1_read(void *buf, int len) {
...
static int ftdi1_dtr(int state) {
	int ret;
	if (state)
		state = 0;
	else
...
static int ftdi1_rts(int state) {
	int ret;
	if (state)
		state = 0;
	else
...
static int ftdi1_open(int vendor, int product,
		const char* serial) {
	if ((ftdi = ftdi_new()) == 0) {
 ...
 */
static int ftdi1_serial(int baudrate) {
	int ret;
	ftdi_usb_reset(ftdi);
...
static int ftdi1_flush(void) {
	return ftdi_usb_purge_buffers(ftdi);
}

static int ftdi1_mpsse(void) {
	int ret = 0;
...
const struct ftdi_io ftdi_io_ftdi1 = {
		.open = ftdi1_open,
		.close = ftdi1_close,
		.serial_mode = ftdi1_serial,
		.mpsse_mode = ftdi1_mpsse,
		.read = ftdi1_read,
		.write = ftdi1_write,
		.flush = ftdi1_flush,
		.setdtr = ftdi1_dtr,
